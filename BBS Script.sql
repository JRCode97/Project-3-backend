...
CREATE or REPLACE PROCEDURE setup_db() AS
BEGIN
CREATE TABLE `application` (
`application_id` int(11) NOT NULL AUTO_INCREMENT,
`application_title` varchar(1000) NOT NULL,
`application_git_link` varchar(1000) DEFAULT NULL,
PRIMARY KEY (`application_id`),
UNIQUE KEY `application_UN` (`application_title`)
	...
	CREATE TABLE `solution` (
	  ...
	  KEY `FK_bug_report_id` (`bug_report_id`),
	  KEY `FK_client_id_solution` (`solver_client_id`),
...
CREATE or REPLACE PROCEDURE teardown_db() AS
BEGIN
...
<p>But I am putting this into my HTML it is getting printing as is:</p>','Nuria','Medium','High','Requested','Line 47',1590535187867,1590535187867,1590610086930,10,1);
INSERT INTO bug_report(bug_report_id,bug_report_title,bug_report_description,bug_report_reproduction_steps,client_username,severity,priority,status,location,approved_time,resolved_time,date_created,point_value,application_id) VALUES (176,'Recyclerview not scrolling inside Nested ScrollView','I am trying to set the date and time of a linux system from a remote system using Java. In order to do that I have created a server to accept time from the remote system as:','I am trying to use the update query with the LIMIT clause using sqlite-JDBC. Let''s say there are 100 bob''s in the table but I only want to update one of the records. Sample code: String name1 = "..','Nuria','High','High','Requested','Line 53',1590535187867,1590535187867,1590535187867,10,1);
INSERT INTO bug_report(bug_report_id,bug_report_title,bug_report_description,bug_report_reproduction_steps,client_username,severity,priority,status,location,approved_time,resolved_time,date_created,point_value,application_id) VALUES (177,'Can I find out userid of the remote person logged into windows desktop?','I am running a Java Desktop appplication on one of our windows machine. And to make any manual changes in our application, one of our Support persons remote login from his machine using mstsc','I am trying to use the update query with the LIMIT clause using sqlite-JDBC. Let''s say there are 100 bob''s in the table but I only want to update one of the records. Sample code: String name1 = "..','Nuria','High','High','Requested','Line 53',1590535187867,1590535187867,1590535187867,10,1);
INSERT INTO bug_report(bug_report_id,bug_report_title,bug_report_description,bug_report_reproduction_steps,client_username,severity,priority,status,location,approved_time,resolved_time,date_created,point_value,application_id) VALUES (178,'Access to root application support macos java','I need to save data inside the /Library/Application Support/ of macOS within a java application. To access the user application data, I know I can use System.getProperty("user.home") but I want the','I am trying to use the update query with the LIMIT clause using sqlite-JDBC. Let''s say there are 100 bob''s in the table but I only want to update one of the records. Sample code: String name1 = "..','Nuria','High','High','Requested','Line 53',1590535187867,1590535187867,1590535187867,10,1);
INSERT INTO bug_report(bug_report_id,bug_report_title,bug_report_description,bug_report_reproduction_steps,client_username,severity,priority,status,location,approved_time,resolved_time,date_created,point_value,application_id) VALUES (179,'Polymorphic association in PostgreSQL with JPA/Hibernate','I have three entities video_guide text_guide and discussion that represent type of learning material available on the platform. A student can create topic-wise boards and add learning material to them','I am trying to use the update query with the LIMIT clause using sqlite-JDBC. Let''s say there are 100 bob''s in the table but I only want to update one of the records. Sample code: String name1 = "..','TheRaidman','High','High','Requested','Line 53',1590535187867,1590535187867,1590535187867,10,1);
INSERT INTO bug_report(bug_report_id,bug_report_title,bug_report_description,bug_report_reproduction_steps,client_username,severity,priority,status,location,approved_time,resolved_time,date_created,point_value,application_id) VALUES (180,'Recyclerview not scrolling inside Nested ScrollView','I have Recyclerview which is under Coordinatorlayout > NestedScrollview > ViewPager and ViewPager has 3 fragment, one has image gallery which is working with the help of Recyclerview.','I have Recyclerview which is under Coordinatorlayout > NestedScrollview > ViewPager and ViewPager has 3 fragment, one has image gallery which is working with the help of Recyclerview.','TheRaidman','High','High','Unresolved','Line 64',1590535187867,1590604344846,1590535084847,10,1);
INSERT INTO bug_report(bug_report_id,bug_report_title,bug_report_description,bug_report_reproduction_steps,client_username,severity,priority,status,location,approved_time,resolved_time,date_created,point_value,application_id) VALUES (184,'Help I can''t type wael','<p><em><strong>Is it well?</strong></em></p>',NULL,'Nuria','Low','High','Requested','in the title',0,0,1590675898008,0,2);
...
<p>If you can tell a little bit, I would like to help. I have experience with video processing.</p>','Nuria','Medium','Low','Resolved','Swift encoder',1590535187867,1590765529139,1590678520634,400,238);
INSERT INTO bug_report(bug_report_id,bug_report_title,bug_report_description,bug_report_reproduction_steps,client_username,severity,priority,status,location,approved_time,resolved_time,date_created,point_value,application_id) VALUES (193,'A new bug','<p>All the formatting is<em> saved</em>, and inserted <strong>html </strong>is character escaped for safety.</p>',NULL,'Nuria','Medium','High','Requested','line 42 of a file',0,0,1590765700829,0,424);
INSERT INTO bug_report(bug_report_id,bug_report_title,bug_report_description,bug_report_reproduction_steps,client_username,severity,priority,status,location,approved_time,resolved_time,date_created,point_value,application_id) VALUES (194,'The list of toppings isn''t showing up','<p>When I try to submit an order, I get a weird "CORS" error. I don''t know what this means!</p>','<p>Any time I try to make a pull request for my list of toppings to show up, it gives me an error and none of my data shows up. Happens any time I try to open any page that should show a list of toppings.</p>','mo','Medium','High','Requested','Javascript',0,0,1590864217119,0,2);
